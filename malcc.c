...
void print_help() {
  printf(
    "Usage: malcc [options] [filename] [outfile]\n"
    "\n"
...
void aot_compile(MalType *arg_vec) {
  if (mal_vector_len(arg_vec) < 3) {
    print_help();
  ...
  MalType *ast = read_str(mal_sprintf("(do %s %S)", builtin_defs, contents)->str);
  struct codegen code = { mal_string(""), mal_string(""), mal_string("") };
  int var_num = 1;
  if (gen_code(ast, env, &code, 1, &var_num, 0)) {
    MalType *out = mal_sprintf("%s\n%s\n", program_template, aot_template);
    out = mal_string_replace(out, "{{TOP_CODE}}", code.top->str);
    MalType *decl_and_body = code.decl;
    mal_string_append_mal_string(decl_and_body, code.body);
    out = mal_string_replace(out, "{{EVAL_CODE}}", decl_and_body->str);
...
int main(int argc, char *argv[]) {
  ...
  if (mal_vector_len(arg_vec) >= 1) {
    if (strcmp(mal_vector_ref(arg_vec, 0)->str, "--compile") == 0) {
      aot_compile(arg_vec);
      exit(0);
    } else if (strcmp(mal_vector_ref(arg_vec, 0)->str, "--help") == 0) {
      print_help();
      exit(0);
    }
  }
